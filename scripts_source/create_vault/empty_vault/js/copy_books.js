...
module.exports = async (params) => {
	...
	async function chooseBookFromModal(books_folder) {
		...
		return await params.quickAddApi.suggester(book_names, book_names);
	}
	...
	async function copyBook(book_name) {
		const input_book_path = app.vault.getFolderByPath(`${BOOKS_FOLDER_NAME}/${book_name}`);
		...
		let output_dir_path = app.vault.getFolderByPath(SCRIPTS_FOLDER_NAME);
		
		if (output_dir_path === null) {
			output_dir_path = await app.vault.createFolder(SCRIPTS_FOLDER_NAME);
		}

		const output_book_path = app.vault.getFolderByPath(`${SCRIPTS_FOLDER_NAME}/${book_name}`);

		if (output_book_path !== null) {
			new Notice(`${output_book_path.path} folder already exists`);
			return false;
		}
